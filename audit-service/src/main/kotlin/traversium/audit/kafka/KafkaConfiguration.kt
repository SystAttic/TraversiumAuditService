...
/**
 * Kafka configuration for audit service
 * @author Ozbej Pavc
 */
@Configuration
@RefreshScope
...
class KafkaConfiguration(val kafkaProperties: KafkaProperties) {
    ...
    @Bean
    fun auditKafkaConsumer(auditService: AuditService) = KafkaConsumer(auditService)

    @Bean
    fun consumerFactory(
        @Qualifier("kafkaObjectMapper") kafkaObjectMapper: ObjectMapper
    ): ConsumerFactory<String, AuditStreamData> {
        return DefaultKafkaConsumerFactory(
            kafkaConsumerConfig(
                kafkaProperties.bootstrapServers,
                "${kafkaProperties.consumerGroupPrefix?.let { "$it-" } ?: ""}audit-group",
                kafkaProperties.fetchMaxBytes,
                kafkaProperties.maxPartitionFetchBytes,
                AuditStreamData::class
            ),
            StringDeserializer(),
            JsonDeserializer(AuditStreamData::class.java, kafkaObjectMapper)
        )
    }

    @Bean
    fun auditEventListenerContainer(
        auditKafkaConsumer: KafkaConsumer,
        consumerFactory: ConsumerFactory<String, AuditStreamData>
    ): KafkaMessageListenerContainer<String, AuditStreamData> = KafkaMessageListenerContainer(
        consumerFactory,
